...
- 2026-06: (1) Profil Usaha di nota — Settings diperluas (nama_usaha, alamat, telepon, logo dataURL) via dialog Pengaturan (upload logo maks 500KB); nota memakai logo & profil. (2) Diskon Persen — toggle Rp/% di dialog Jual, diskon dihitung ke rupiah otomatis. (3) Export Penjualan — tombol Export CSV di Riwayat Nota (BOM UTF-8 untuk Excel). (4) Samakan Margin — tombol per kategori di tab HPP, hitung ulang harga jual semua produk dari HPP sesuai target margin (produk tanpa HPP dilewati).
- 2026-06: (1) Stok Produk — kolom stok editable per produk (Product.stok), berkurang otomatis (server $inc) tiap penjualan sesuai qty; stok<=0 ditandai merah. (2) Rekap Penjualan PDF — lib/salesReport.js, tombol "Rekap PDF" + dialog pilih bulan di Riwayat Nota; unduh HTML rapi (profil usaha, ringkasan omzet/diskon/laba, per-kategori, tabel transaksi) siap Cetak/Simpan PDF. (3) Persen Diskon di Nota — baris diskon pada nota kini menampilkan persentase terhitung selain nominal rupiah.
